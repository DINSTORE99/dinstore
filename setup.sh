...
check_license() {
    ...
    if [[ -z "${MYIP}" ]]; then
        echo -e "${RED}IP VPS tidak dapat dideteksi.${NC}"
        exit 1
    fi

    local data line ip exp label today
    data=$(curl -4 -fsS --max-time 15 "$LICENSE_URL" 2>/dev/null || true)
    if [[ -z "$data" ]]; then
        echo -e "${RED}Gagal mengambil database license dari GitHub.${NC}"
        echo "Periksa koneksi internet atau LICENSE_URL."
        exit 1
    fi

    today=$(date +%Y-%m-%d)
    line=$(printf '%s\n' "$data" | awk -F'|' -v ip="$MYIP" '$1 == ip {print; exit}')

    if [[ -z "$line" ]]; then
        echo -e "${RED}STATUS : NOT REGISTERED${NC}"
        echo
        echo "IP ini belum mendapatkan izin DINSTORE."
        echo
        echo "Tambahkan IP VPS ke license/licenses.txt di GitHub."
        exit 1
    fi

    IFS='|' read -r ip exp label <<< "$line"

    if ! date -d "$exp" +%s >/dev/null 2>&1; then
        echo -e "${RED}Format tanggal license tidak valid.${NC}"
        exit 1
    fi
    ...
    local days=$(( ( $(date -d "$exp" +%s) - $(date -d "$today" +%s) ) / 86400 ))
    printf '%s\n' "$line" > "$LICENSE_FILE"
    chmod 600 "$LICENSE_FILE"

    echo -e "${GREEN}STATUS  : ACTIVE${FONT}"
    echo "Label   : ${label:--}"
    echo "Expired : $exp"
    echo "Sisa    : ${days} hari"
    echo
...
REPO="https://raw.githubusercontent.com/DINSTORE99/dinstore/main/"
start=$(date +%s)
secs_to_human() {
...
echo ""
echo ""
echo -e "\033[1;32mDINSTORE siap digunakan.${FONT}"
sleep 2
clear
if command -v menu >/dev/null 2>&1; then
    exec menu
else
    echo "Perintah menu belum ditemukan."
    echo "Cek: /usr/local/sbin/menu"
fi
